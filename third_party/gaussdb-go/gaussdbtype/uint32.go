...
// decodeUint32Payload decodes a Uint32Codec payload. GaussDB sends 8-byte
// payloads for some OID-family columns (e.g. pg_database.datfrozenxid64 in
// MySQL-compatible mode), so both 4-byte and 8-byte lengths are accepted.
// 8-byte values that exceed uint32 range are rejected to avoid truncation.
func decodeUint32Payload(src []byte) (uint64, error) {
	switch len(src) {
	case 4:
	...
	case 8:
		...
		return v, nil
	default:
		return 0, fmt.Errorf("invalid length for uint32: %v", len(src))
	}
}
