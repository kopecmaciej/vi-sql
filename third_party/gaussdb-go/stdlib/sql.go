		...
		for i, fd := range fieldDescriptions {
			...
			case gaussdbtype.CIDOID, gaussdbtype.OIDOID, gaussdbtype.XIDOID:
				...
				r.valueFuncs[i] = func(src []byte) (driver.Value, error) {
					err := scanPlan.Scan(src, &d)
					if err != nil {
						// GaussDB sends int8-sized payloads for some XID-family
						// columns (e.g. pg_database.datfrozenxid64 in MySQL-compatible
						// mode). Values that exceed uint32 range cannot be represented
						// by the uint32 codec, so surface them as text.
						if len(src) == 8 {
							return strconv.FormatUint(binary.BigEndian.Uint64(src), 10), nil
						}
						return nil, err
					}
					return d.Value()
